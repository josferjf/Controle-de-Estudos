...
// ============================================================
        ...
        function saveWeeklyDistribution() {
            ...
            updateUI();
            alert("Carga horária salva com sucesso! As metas da semana foram geradas com base na nova distribuição.");
        }
        ...
        function updateExamCountdownAndPace() {
            ...
            if (remT <= 0) {
                statusEl.innerHTML = `<div class="alert-banner" style="background-color: var(--primary-alpha); border:1px solid rgba(198,234,60,0.3); color: var(--primary); margin:0;"><i data-lucide="check-circle"></i> <span>Edital concluído!</span></div>`;
            } else if (currentPace + 0.05 >= requiredPace) {
                statusEl.innerHTML = `<div class="alert-banner" style="background-color: var(--primary-alpha); border:1px solid rgba(198,234,60,0.3); color: var(--primary); margin:0;"><i data-lucide="check-circle"></i> <span>Seu ritmo está em dia com o prazo da prova.</span></div>`;
            } else {
        ...
        function saveGlobalConfiguration() {
            ...
            regenerateSmartCycle(false);
            updateUI();
            alert("Configurações do ciclo updated globalmente.");
        }
        ...
        function simulateInactivityDelay() {
            ...
            if (missedHours <= 0.05) {
                alert("Nenhum atraso detectado nos últimos 14 dias! Seu ritmo está em dia com a distribuição configurada.");
            } else {
                alert(`Atraso detectado: ${missedHours.toFixed(1)}h não estudadas em ${missedDaysCount} dia(s) nos últimos 14 dias, considerando sua distribuição configurada. Recomenda-se reforçar aprox. ${(missedHours / 7).toFixed(1)}h extras por dia nos próximos 7 dias para recuperar o ritmo.`);
            }
