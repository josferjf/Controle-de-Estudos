...
// ============================================================
        ...
        function addTopicToSubject(subjectId) {
            ...
            const subj = appState.subjects.find(s => s.id === subjectId);
            if (!subj) return;
            subj.topics.push({ id: `top-${Date.now()}`, title: title, completed: false, link: '' });
            input.value = "";
            regenerateSmartCycle(false);
            updateUI();
        }
            ...
            if (editId) {
                ...
                if(topicsText) {
                    topicsArr = topicsText.split('\n').map((t, idx) => {
                        return { id: `top-${Date.now()}-${idx}`, title: t.trim(), completed: false, link: '' };
                    });
                } else {
                    topicsArr = [{ id: `top-${Date.now()}-0`, title: "Aula Geral 00", completed: false, link: '' }];
                }
