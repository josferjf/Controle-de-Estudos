...
// ============================================================
        ...
        function exportErrorNotebookToCSV() {
            const headers = ['Data', 'Materia', 'O_Que_Errei', 'Causa', 'Regra_Certa', 'Reincidencias', 'Vezes_Revisado', 'Ultima_Revisao'];
            const rows = appState.error_notebook.map(err => [
                ...
                err.view_count || 0,
                err.last_viewed_at ? new Date(err.last_viewed_at).toLocaleDateString('pt-BR') : ''
            ]);
        ...
        function renderBackupStatus() {
            ...
            if (daysSince > 7) {
                ...
                el.innerHTML = `<span style="color: var(--primary-text);">Último backup: ${formattedDate} (há ${daysSince} dia${daysSince === 1 ? '' : 's'}).</span>`;
            }
        }
        ...
        function importBackupJSON(event) {
            ...
            r.onload = async function(e) {
                ...
                try {
                    ...
                    customAlert("Não foi possível ler este arquivo: o JSON está corrompido ou mal formatado.");
                    event.target.value = "";
                    return;
                }

                if (!parsed.subjects || !parsed.study_logs) {
                    customAlert("Este arquivo não parece ser um backup válido do Gerenciador de Estudos (faltam campos essenciais como 'subjects' ou 'study_logs').");
                    event.target.value = "";
                    return;
                }
                ...
                updateUI();
                renderBackupStatus();
                customAlert("Backup restaurado com sucesso!");
                event.target.value = "";
            };
            r.readAsText(file);
        }
        ...
        async function clearAllData() {
            const firstConfirmation = await customConfirm("ATENÇÃO! Você tem certeza de que deseja apagar permanentemente todas as matérias, logs, históricos e resumos cadastrados?");
            if (firstConfirmation) {
                const secondConfirmation = await customConfirm("CONFIRMAÇÃO FINAL: Esta ação não pode ser desfeita e limpará todos os seus dados salvos na nuvem. Deseja continuar?");
                if (secondConfirmation) {
                    ...
                    renderBackupStatus();
                    switchTab('focus');
                    customAlert("O aplicativo foi redefinido com sucesso para o estado original de fábrica.");
                }
            }
        }

        // --- ALTERAÇÃO 5: EXCLUSÃO INDEPENDENTE APENAS DAS ATIVIDADES CONCLUÍDAS ---
        async function clearCompletedActivities() {
            const confirmation = await customConfirm("Deseja excluir permanentemente apenas o histórico de atividades concluídas (sessões de estudo registradas)? Disciplinas, ciclos, metas pendentes, carga horária e configurações serão mantidos.");
            if (confirmation) {
                appState.study_logs = [];
                saveToDatabase();
                updateUI();
                ...
                customAlert("Histórico de atividades concluídas removido com sucesso. O restante do sistema foi mantido.");
            }
        }
