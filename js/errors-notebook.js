...
// ============================================================
        ...
        function addManualError() {
            ...
            if (!title || !text) { alert("Preencha o título e o texto explicativo!"); return; }
            ...
            if (fileInput.files && fileInput.files[0]) {
                ...
                reader.onload = function(e) {
                    ...
                    alert("Sucesso! Erro/Print adicionado com sucesso.");
                };
                reader.readAsDataURL(fileInput.files[0]);
            } else {
                finalizeErrorCreation();
                alert("Sucesso! Erro textual registrado.");
            }
        ...
        function deleteErrorItem(errorId) {
            if (confirm("Deseja realmente remover permanentemente este item do seu caderno de erros?")) {
            ...
            filteredList.forEach(err => {
                ...
                card.innerHTML = `
                    ...
                    <div style="display:flex; justify-content:space-between; align-items:center; margin-top:12px; padding-top:10px; border-top: 1px solid var(--border);">
                        ...
                        <button class="filter-chip" style="padding: 4px 10px; background: var(--primary-alpha); color: var(--primary); font-size: 11px;" onclick="markErrorAsReviewed('${err.id}')"><i data-lucide="check" style="width:12px; height:12px; vertical-align:middle;"></i> Marcar como Revisado</button>
                    </div>
                `;
                ...
                chartErrorsRootCauseInstance = new Chart(rootCauseCanvas.getContext('2d'), {
                    ...
                    data: {
                        ...
                        datasets: [{
                            ...
                            backgroundColor: ['#5a5a50', '#ef4444', '#f59e0b', '#3b82f6', '#8b5cf6', '#c6ea3c']
                        }]
                    },
                    options: {
        ...
        function openFlashcardMode() {
            ...
            if (list.length === 0) {
                alert("Nenhum erro encontrado com os filtros atuais para revisar.");
                return;
            }
        ...
        function exportErrorsToPrintablePDF() {
            const list = getFilteredErrorNotebook();
            if (list.length === 0) {
                alert("Nenhum erro encontrado com os filtros atuais para exportar.");
                return;
            }
