        ...
        function updateUI() {
            ...
            if (currentStep) {
                ...
                if (currentStep.topicLink) {
                    linkButtonsHtml += `<a href="${escapeHTML(currentStep.topicLink)}" target="_blank" rel="noopener" class="filter-chip topic-link-btn" style="display:inline-flex; padding: 10px 18px; background: var(--primary-alpha); text-decoration:none; font-size:15px;"><i data-lucide="link" style="width:16px; height:16px;"></i>&nbsp;Caderno de Questões</a>`;
                }
                if (currentStep.topicMaterialLink) {
                    linkButtonsHtml += `<a href="${escapeHTML(currentStep.topicMaterialLink)}" target="_blank" rel="noopener" class="filter-chip topic-link-btn" style="display:inline-flex; padding: 10px 18px; background: var(--primary-alpha); text-decoration:none; font-size:15px;"><i data-lucide="book-open" style="width:16px; height:16px;"></i>&nbsp;Material Teórico</a>`;
                }
            ...
            itemsSlice.forEach(([key, data]) => {
                ...
                item.innerHTML = `<div style="min-width:0; flex:1;"><strong>${escapeHTML(step.subjectName)}</strong>${freqBadge}<br><small title="${escapeHTML(step.topicTitle)}" style="color:var(--text-muted); display:-webkit-box; -webkit-line-clamp:2; -webkit-box-orient:vertical; overflow:hidden;">${escapeHTML(step.topicTitle)}</small></div>
