...
// ============================================================
        ...
        function editTopicLink(subjectId, topicId) {
            const subj = appState.subjects.find(s => s.id === subjectId);
            if (!subj) return;
            const topic = subj.topics.find(t => t.id === topicId);
            if (!topic) return;
            const newLink = prompt("Cole o link do caderno de questões para este tópico (deixe em branco para remover):", topic.link || '');
