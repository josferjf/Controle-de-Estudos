...
// ============================================================
        ...
        function updateUI() {
            ...
            if (currentStep) {
                ...
                if (currentStep.topicLink) {
                    linkButtonsHtml += `<a href="${escapeHTML(currentStep.topicLink)}" target="_blank" rel="noopener" class="filter-chip topic-link-btn" style="display:inline-flex; padding: 7px 14px; background: var(--primary-alpha); text-decoration:none; font-size:13px;"><i data-lucide="link" style="width:13px; height:13px;"></i>&nbsp;Caderno de Questões</a>`;
                }
            ...
            itemsSlice.forEach(([key, data]) => {
                ...
                const finalStretchBadge = step.finalStretchReason ? `<span class="badge badge-danger" style="margin-left:6px; white-space:nowrap; display:inline-block;">${step.finalStretchReason === 'erros' ? '🔥 Erro recorrente' : '🔥 Abaixo da meta'}</span>` : '';
                const linkIcon = step.topicLink ? `<a href="${escapeHTML(step.topicLink)}" target="_blank" rel="noopener" onclick="event.stopPropagation();" title="Caderno de Questões" style="color: var(--primary-text); display:inline-flex;"><i data-lucide="link" style="width:14px; height:14px;"></i></a>` : '';
                const materialIcon = step.topicMaterialLink ? `<a href="${escapeHTML(step.topicMaterialLink)}" target="_blank" rel="noopener" onclick="event.stopPropagation();" title="Material Teórico" style="color: var(--primary-text); display:inline-flex;"><i data-lucide="book-open" style="width:14px; height:14px;"></i></a>` : '';
                item.innerHTML = `<div style="min-width:0; flex:1;"><strong>${escapeHTML(step.subjectName)}</strong>${freqBadge}${finalStretchBadge}<br><small title="${escapeHTML(step.topicTitle)}" style="color:var(--text-muted); display:-webkit-box; -webkit-line-clamp:2; -webkit-box-orient:vertical; overflow:hidden;">${escapeHTML(step.topicTitle)}</small></div>
            ...
            // Modo Reta Final: reflete o estado atual no botão e no banner de aviso
            const finalStretchActive = !!appState.user_configuration.final_stretch_mode;
            const finalStretchBtn = document.getElementById('btn-final-stretch-toggle');
            const finalStretchBanner = document.getElementById('final-stretch-active-banner');
            if (finalStretchBtn) {
                finalStretchBtn.innerHTML = finalStretchActive
                    ? '<i data-lucide="flame" style="width:12px; height:12px;"></i> Desativar Reta Final'
                    : '<i data-lucide="flame" style="width:12px; height:12px;"></i> Modo Reta Final';
                finalStretchBtn.style.background = finalStretchActive ? 'var(--danger-alpha)' : '';
                finalStretchBtn.style.color = finalStretchActive ? 'var(--danger)' : '';
            }
            if (finalStretchBanner) finalStretchBanner.style.display = finalStretchActive ? 'flex' : 'none';
        ...
        function renderCharts() {
            ...
            if(chartTopicBarInstance) chartTopicBarInstance.destroy();
            ...
            // Desempenho por Tópico: só faz sentido dentro de uma matéria específica (senão os tópicos
            // de matérias diferentes se misturariam no mesmo eixo). Só entram sessões com questões reais
            // (sessões só teoria não têm % de acerto pra comparar).
            const ctxTopicBarCanvas = document.getElementById('canvas-topic-bar');
            const topicBarNoticeEl = document.getElementById('topic-bar-notice');
            if (ctxTopicBarCanvas) {
                if (selectedSubjectFilter === 'all') {
                    ctxTopicBarCanvas.style.display = 'none';
                    if (topicBarNoticeEl) {
                        topicBarNoticeEl.style.display = 'block';
                        topicBarNoticeEl.innerText = 'Selecione uma matéria específica no filtro acima para ver o desempenho por tópico.';
                    }
                } else {
                    const topicPerfMap = {};
                    filteredLogs
                        .filter(l => l.snapshot_subject_name === selectedSubjectFilter && !l.is_theory_only && l.questions_attempted > 0)
                        .forEach(l => {
                            if (!topicPerfMap[l.snapshot_topic_title]) topicPerfMap[l.snapshot_topic_title] = { questions: 0, correct: 0 };
                            topicPerfMap[l.snapshot_topic_title].questions += l.questions_attempted;
                            topicPerfMap[l.snapshot_topic_title].correct += l.questions_correct;
                        });
                    const topicLabels = Object.keys(topicPerfMap);
                    if (topicLabels.length === 0) {
                        ctxTopicBarCanvas.style.display = 'none';
                        if (topicBarNoticeEl) {
                            topicBarNoticeEl.style.display = 'block';
                            topicBarNoticeEl.innerText = 'Ainda não há sessões com questões registradas para os tópicos dessa matéria.';
                        }
                    } else {
                        ctxTopicBarCanvas.style.display = 'block';
                        if (topicBarNoticeEl) topicBarNoticeEl.style.display = 'none';
                        const topicPerfData = topicLabels.map(t => Math.round((topicPerfMap[t].correct / topicPerfMap[t].questions) * 100));
                        const targetScoreForTopics = appState.user_configuration.target_score || 85;
                        chartTopicBarInstance = new Chart(ctxTopicBarCanvas.getContext('2d'), {
                            type: 'bar',
                            data: {
                                labels: topicLabels,
                                datasets: [{
                                    label: 'Aproveitamento por Tópico (%)',
                                    data: topicPerfData,
                                    backgroundColor: topicPerfData.map(p => p < targetScoreForTopics ? 'rgba(239, 68, 68, 0.75)' : 'rgba(34, 197, 94, 0.75)'),
                                    borderRadius: 4
                                }, {
                                    label: `Nota de Corte (${targetScoreForTopics}%)`,
                                    type: 'line',
                                    data: topicLabels.map(() => targetScoreForTopics),
                                    borderColor: '#ef4444',
                                    borderDash: [6, 4],
                                    borderWidth: 1.5,
                                    pointRadius: 0,
