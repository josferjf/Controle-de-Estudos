...
// ============================================================
        ...
        function handlePomodoroPhaseComplete() {
            ...
            if (phase === 'focus') {
                ...
                appState.timer_state.startTime = Date.now();
                alert(`Bloco de foco concluído! Iniciando ${isLongBreak ? 'uma pausa longa (15 min)' : 'uma pausa curta (5 min)'}.`);
                executeTimerLoop();
            } else {
                appState.timer_state.pomodoro_phase = 'focus';
                appState.timer_state.seconds = POMODORO_FOCUS_SECONDS;
                ...
                appState.timer_state.isRunning = false;
                appState.timer_state.startTime = null;
                alert("Pausa concluída! Quando estiver pronto, inicie o próximo bloco de foco.");
        ...
        function skipCurrentCycleStep() {
            ...
            if (appState.timer_state.isRunning) {
                alert("Pause o cronômetro antes de pular este item.");
                return;
            }
            if (!confirm("Pular este item do ciclo sem registrar sessão? Ele continuará pendente e poderá aparecer novamente.")) return;
