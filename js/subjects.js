...
// ============================================================
        ...
        function toggleTopicMenu(event, topicId) {
            ...
            // Como o menu agora é position:fixed, a posição é calculada em pixels reais da janela,
            // sempre travada (clamp) pra nunca vazar da tela em nenhuma direção, não importa onde o
            // botão esteja (dentro de listas, cards com hover, ou perto da borda da tela).
            lucide.createIcons();
            ...
            appState.subjects.forEach((s, idx) => {
                ...
                const topicsHtml = s.topics.map((t, tIdx) => `
                    <div style="display:flex; align-items:center; gap:8px; padding: 6px 0; border-bottom: 1px solid var(--border); width:100%; max-width:100%; min-width:0;">
                        ...
                        <span style="flex:1; font-size:13px; min-width:0; overflow:hidden; text-overflow:ellipsis; white-space:nowrap; ${t.completed ? 'color:var(--text-muted); text-decoration:line-through;' : ''}" title="${escapeHTML(t.title)}">${escapeHTML(t.title)}</span>
            ...
            if (editId) {
                ...
                if(topicsText) {
                    topicsArr = topicsText.split('\n')
                        ...
                        .filter(t => t !== "")
                        .map((t, idx) => {
                            return { id: `top-${Date.now()}-${idx}`, title: t, completed: false, link: '', materialLink: '' };
                        });
        ...
        function editSubject(subjectId) {
            ...
            document.getElementById('form-subject-title-context').innerHTML = `<span class="title-text" style="color: var(--warning);">Modo Edição: ${escapeHTML(subject.name)}</span>`;
