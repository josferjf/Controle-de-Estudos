...
// ============================================================
        ...
        function renameTopic(subjectId, topicId) {
            ...
            const topic = subj.topics.find(t => t.id === topicId);
            if (!topic) return;
            const newTitle = prompt("Renomear tópico/aula:", topic.title);
        ...
        function deleteTopic(subjectId, topicId) {
            const subj = appState.subjects.find(s => s.id === subjectId);
            if (!subj) return;
            if (subj.topics.length <= 1) { alert("A matéria precisa ter pelo menos um tópico/aula."); return; }
            if (confirm("Remover este tópico/aula permanentemente?")) {
        ...
        function addTopicToSubject(subjectId) {
            ...
            if (!title) { alert("Digite o nome do novo tópico/aula."); return; }
            const subj = appState.subjects.find(s => s.id === subjectId);
            if (!subj) return;
            subj.topics.push({ id: `top-${Date.now()}`, title: title, completed: false });
            input.value = "";
            regenerateSmartCycle(false);
            updateUI();
        }
        ...
        function addSubject() {
            ...
            if(!name) { alert("Insira o nome da matéria."); return; }
            ...
            if (editId) {
                ...
                if(topicsText) {
                    topicsArr = topicsText.split('\n').map((t, idx) => {
                        return { id: `top-${Date.now()}-${idx}`, title: t.trim(), completed: false };
                    });
                } else {
                    topicsArr = [{ id: `top-${Date.now()}-0`, title: "Aula Geral 00", completed: false }];
                }
                ...
                alert("Nova matéria adicionada ao ciclo com sucesso!");
            }
        ...
        function deleteSubject(subjectId) {
            if (confirm("ATENÇÃO: Remover esta disciplina excluirá todas as aulas e metas pendentes vinculadas a ela. Os logs históricos de estudo salvos serão preservados. Deseja continuar?")) {
