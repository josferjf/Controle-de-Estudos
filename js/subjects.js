            ...
            appState.subjects.forEach((s, idx) => {
                ...
                const topicsHtml = s.topics.map((t, tIdx) => `
                    <div style="display:flex; align-items:center; gap:8px; padding: 6px 0; border-bottom: 1px solid var(--border);">
                        <input type="checkbox" style="width:auto;" ${t.completed ? 'checked' : ''} onchange="toggleTopicCompleted('${s.id}','${t.id}')">
                        <span style="flex:1; font-size:13px; ${t.completed ? 'color:var(--text-muted); text-decoration:line-through;' : ''}">${t.title}</span>
                        <button class="filter-chip" style="padding:3px 5px; background: var(--bg-card); font-size:10px;" onclick="moveTopic('${s.id}','${t.id}', -1)" ${tIdx === 0 ? 'disabled' : ''} title="Mover pra cima"><i data-lucide="chevron-up" style="width:11px; height:11px;"></i></button>
                        <button class="filter-chip" style="padding:3px 5px; background: var(--bg-card); font-size:10px;" onclick="moveTopic('${s.id}','${t.id}', 1)" ${tIdx === s.topics.length - 1 ? 'disabled' : ''} title="Mover pra baixo"><i data-lucide="chevron-down" style="width:11px; height:11px;"></i></button>
                        <button class="filter-chip" style="padding:3px 6px; background: ${t.link ? 'var(--primary-alpha)' : 'var(--bg-card)'}; color: ${t.link ? 'var(--primary)' : 'var(--text-main)'}; font-size:10px;" onclick="editTopicLink('${s.id}','${t.id}')" title="${t.link ? 'Editar link do caderno de questões' : 'Adicionar link do caderno de questões'}"><i data-lucide="link" style="width:11px; height:11px;"></i></button>
                        <button class="filter-chip" style="padding:3px 6px; background: ${t.materialLink ? 'var(--primary-alpha)' : 'var(--bg-card)'}; color: ${t.materialLink ? 'var(--primary)' : 'var(--text-main)'}; font-size:10px;" onclick="editTopicMaterialLink('${s.id}','${t.id}')" title="${t.materialLink ? 'Editar link do material teórico' : 'Adicionar link do material teórico'}"><i data-lucide="book-open" style="width:11px; height:11px;"></i></button>
                        <button class="filter-chip" style="padding:3px 6px; background: var(--bg-card); font-size:10px;" onclick="renameTopic('${s.id}','${t.id}')"><i data-lucide="pencil" style="width:11px; height:11px;"></i></button>
                        <button class="filter-chip" style="padding:3px 6px; background: var(--danger-alpha); color:var(--danger); font-size:10px;" onclick="deleteTopic('${s.id}','${t.id}')"><i data-lucide="x" style="width:11px; height:11px;"></i></button>
                    </div>
                `).join('');
                ...
                div.innerHTML = `
                    ...
                    <div style="display:flex; gap:8px; margin-top:12px;">
                        ...
                        <button class="filter-chip" style="padding: 5px 10px; background: var(--bg-card); font-size: 11px;" onclick="toggleSubjectTopicsExpanded('${s.id}')"><i data-lucide="${isExpanded ? 'chevron-up' : 'list'}" style="width:12px; height:12px; vertical-align:middle;"></i> ${isExpanded ? 'Ocultar Aulas' : 'Ver/Gerenciar Aulas'}</button>
                    </div>
                    ${isExpanded ? `
